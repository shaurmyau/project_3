...
def run_rabbitmq(args):
    ...
    # Увеличенный prefetch – до 5000
    prefetch = getattr(args, 'prefetch', 5000)
    channel.basic_qos(prefetch_count=prefetch)
    channel.basic_consume(queue=args.queue, on_message_callback=on_message, auto_ack=True)

    print(f"[Consumer] RabbitMQ start. Prefetch={prefetch}. Waiting...", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('--prefetch', type=int, default=5000, help='RabbitMQ prefetch count')

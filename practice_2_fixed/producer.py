...
def rabbitmq_worker(args, thread_id, thread_rate, results):
    ...
    while time.time() < end_time:
        ...
        try:
            channel.basic_publish(exchange='', routing_key=args.queue, body=msg_body)
            sent += 1
...
def run_rabbitmq_multithreaded(args):
    ...
    with ThreadPoolExecutor(max_workers=total_threads) as executor:
        futures = [executor.submit(rabbitmq_worker, args, i, per_thread_rate, results) for i in range(total_threads)]
...
# ---------- Redis (многопоточный) ----------
def redis_worker(args, thread_id, thread_rate, results):
    ...
    while time.time() < end_time:
        ...
        try:
            ...
            failed += 1
            print(f"[Producer-{thread_id}] redis error: {e}", file=sys.stderr)
        elapsed = time.time() - loop_start
        sleep_time = interval - elapsed
        if sleep_time > 0:
            time.sleep(sleep_time)
...
def run_redis_multithreaded(args):
    total_threads = args.threads
    per_thread_rate = max(1, args.rate // total_threads)
    print(f"[Producer] Redis multi ({total_threads} threads): size={args.size}B, "
          f"total_rate={args.rate}/s, per_thread={per_thread_rate}/s, duration={args.duration}s",
          file=sys.stderr)
    results = {}
    with ThreadPoolExecutor(max_workers=total_threads) as executor:
        futures = [executor.submit(redis_worker, args, i, per_thread_rate, results) for i in range(total_threads)]
        for future in as_completed(futures):
            future.result()
    total_sent = sum(r[0] for r in results.values())
    total_failed = sum(r[1] for r in results.values())
...
def main():
    ...
    parser.add_argument('--threads', type=int, default=4, help='Number of producer threads (both brokers)')

...
def run_all_tests():
    ...
    # Вспомогательная функция для безопасного имени
    def safe_name(scenario):
        return scenario.replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '').replace('-', '_')
    ...
    for scenario_name, read_ratio in scenarios:
        ...
        safe_scenario = safe_name(scenario_name)
        
        # Создаём временные файлы в текущей директории (или можно в /tmp)
        db1_path = f"test_db_ca_{safe_scenario}.db"
        db2_path = f"test_db_wt_{safe_scenario}.db"
        db3_path = f"test_db_wb_{safe_scenario}.db"
    ...
    # Итоговая таблица (без изменений, как в исходном коде)
    # ... (оставьте остальной код как был)
